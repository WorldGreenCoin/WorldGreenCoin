...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid WGC address or script");
    }
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by WGC %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
Value bip38encrypt(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"worldGreenCoinaddress\"\n"
            "\nEncrypts a private key corresponding to 'worldGreenCoinaddress'.\n"
            "\nArguments:\n"
            "1. \"worldGreenCoinaddress\"   (string, required) The worldGreenCoin address for the private key (you must hold the key already)\n"
...
Value bip38decrypt(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38decrypt \"worldGreenCoinaddress\"\n"
