...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("WGCMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("WGCMiner : ProcessNewBlock, block not accepted");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("WGCMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running WGCMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadWGCMiner() exception");
    } catch (...) {
        LogPrintf("ThreadWGCMiner() exception");
    }

    LogPrintf("ThreadWGCMiner exiting\n");
}
